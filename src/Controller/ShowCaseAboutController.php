...
use App\Form\ShowCaseAboutType;
...
class ShowCaseAboutController extends AbstractController
{
    #[Route('/show/case/about', name: 'app_admin_show_case_about')]
    public function index(Request $request, EntityManagerInterface $manager, ShowCaseAboutRepository $showCaseAboutRepository): Response
    {
        $error = null;
        $images = null;
        $tab_server = null;
        if ($this->getUser() && $this->getUser()->getRoles()[0] == "ROLE_ADMIN") {
            $id = $this->getUser();
            $homePageSection = $showCaseAboutRepository->findOneBy(["factory" => $id->getFactory()]);
            if ($homePageSection) {

                $images = $homePageSection->getImageName();

                $form = $this->createForm(ShowCaseAboutType::class, $homePageSection);
                ...
                if ($form->isSubmitted() && $form->isValid()) {
                    ...
                    if ($form_files == []) {
                        ...
                        return $this->redirectToRoute("app_admin_show_case_about");
                    }
                ...
                return $this->render('show_case_about/index.html.twig', [
                ...
                $homeSection = new ShowCaseAbout();

                $form = $this->createForm(ShowCaseAboutType::class, $homeSection);
