...
use App\Form\ShowCaseHomePageType;
...
class ShowCaseHomePageController extends AbstractController
{
    #[Route('admin/show/case/home/page', name: 'app_admin_show_case_home_page')]
    public function index(Request $request, EntityManagerInterface $manager, ShowCaseHomePageRepository $showCaseHomePageRepository): Response
    {
        $error = null;
        $images = null;
        $tab_server = null;
        if ($this->getUser() && $this->getUser()->getRoles()[0] == "ROLE_ADMIN") {
            $id = $this->getUser();
            $homePageSection = $showCaseHomePageRepository->findOneBy(["factory" => $id->getFactory()]);
            if ($homePageSection) {

                $images = $homePageSection->getImageName();

                $form = $this->createForm(ShowCaseHomePageType::class, $homePageSection);
                ...
                return $this->render('show_case_home_page/index.html.twig', [
                ...
                $homeSection = new ShowCaseHomePage();

                $form = $this->createForm(ShowCaseHomePageType::class, $homeSection);
